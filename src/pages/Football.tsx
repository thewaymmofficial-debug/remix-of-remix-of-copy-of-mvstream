...
export default function Football() {
  ...
  const replayVideos = useMemo(() => {
    if (!videos) return [];
    return videos.filter(v => v.show_in_replay);
  }, [videos]);

  const liveVideos = useMemo(() => replayVideos.filter(v => v.is_live), [replayVideos]);
  const nonLiveVideos = useMemo(() => replayVideos.filter(v => !v.is_live), [replayVideos]);
